...
- [ ] 2 名ペア 2 つの場合、2 名ペア・3 名ペアになること
- [ ] 3 名ペア 2 つの場合、2 名ペア・2 名ペア・3 名ペアになること
...
- ペアが 1,4 名になったときの、処理はどこに記載すべきか?
  - ユースケースのプライベートメソッド
  - ドメインサービス
  - ドメインモデル
- ペア名は自動で付与された方が良いよなあ
- repo.find 系の戻り値の型はどうしても、Promise<User | null> となると、ユースケースで毎回 find 系の戻り値の Null チェックしないといけない。これはしょうがない？
- 「チームに所属する参加者は n 人以上」という仕様は、どこに記載する？
  - 上記の仕様をドメインサービスに記述するとしたら、ドメインからドメインサービスを呼び出さなければいけない？と思ってしまい、手が止まった
  - 仕様が「チームに所属するペアは 2 つ以上」であれば、簡単なんだけど、、
- ページネーションかつ複数絞込の際の、API 設計なやむ、、
  - query が複数の場合とかは、どうするんだろう
  - query を判断して controller で usecase をハンドリング。それとも汎用的な usecase を作る?
